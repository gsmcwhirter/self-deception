...
Action.prototype.act = function (file){
  this.emit("end");
};

function StatsAction(){}
util.inherits(StatsAction, Action);
StatsAction.prototype.act = function (file){
  var self = this;
  ...
  rstream.on("readable", function (){
    ...
    else if (buffer.toString().match(/done\.\n/g).length > 0) {
  ...
  rstream.on("error", function (err){
    self.emit("error", {error: err, file: file});
  });
  ...
  rstream.on("end", function (){
    ...
    self.emit("data", {data: retData, file: file});
    self.emit("end");
  });
}
...
if (!fs.statSync(args.dir).isDirectory()){
  ...
  fs.readdir(args.dir, function (err, files){
    ...
    var action = new StatsAction();
    var pond = new FilePond(files.map(function (file){ return path.join(args.dir, file); }), action);
    ...
    pond.on("data", function (data){
      console.log("Writing data for %s", data.file);
      
      //wstream.write("Data for " + data.file + "\n", "utf8");
      wstream.write(util.inspect(data, {depth: 4, colors: false}));
      wstream.write("\n\n");
    });
